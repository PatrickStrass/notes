...
import { ChangeEvent, FormEvent, useState, useEffect, KeyboardEvent } from "react"
...
export function NewNoteCard({ onNoteCreated }: NewNoteCardProps) {
  ...
  useEffect(() => {
    window.addEventListener('keydown', handleESQPress)

    return () => {
      window.removeEventListener('keydown', handleESQPress)
    }
  }, [])
  ...
  function handleESQPress(event: KeyboardEvent) {
    if (event.key === "Escape") {
      setShouldShowOnBoarding(true)
      setIsRecording(false)
      setContent("")
      ...
      <Dialog.Portal>
        ...
        <Dialog.Content className="fixed overflow-hidden inset-0 md:inset-auto md:left-1/2 md:top-1/2 md:-translate-x-1/2 md:-translate-y-1/2 md:max-w-[640px] w-full md:h-[60vh] bg-slate-700 md:rounded-md flex flex-col outline-none" >
          <Dialog.Close className="absolute right-0 top-0 bg-slate-800 p-1.5 text-slate-400 hover:text-slate-100">
            <X onClick={handleCloseDialog} className="size-5" />
          </Dialog.Close>
